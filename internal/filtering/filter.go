...
func (d *DNSFilter) updateIntl(ctx context.Context, flt *FilterYAML) (ok bool, err error) {
	...
	if filepath.IsAbs(flt.URL) {
		res, err = d.readFromFile(tmpFile, flt.URL)
	} else {
...
// readFromFile reads filter data from a file located at path and parses it
// into the tmpFile file.  tmpFile must not be nil.  path must be a valid
// filepath.
func (d *DNSFilter) readFromFile(
	...
	// #nosec G304 -- path is checked against safeFSPatterns above.
